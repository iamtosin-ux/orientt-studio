...
type Card = {
  ...
  rotate: number;
  showreel?: boolean;
  /** Links the card to a case study project */
  slug?: string;
};

const CARDS: Card[] = [
  { src: "/work/card-anecdote.png", alt: "Indemni dashboard", rotate: -10.2, slug: "indemni" },
...
export default function ProjectCarousel({ projects }: { projects: ProjectMeta[] }) {
  ...
  return (
    <>
      ...
      >
        {CARDS.map((card, i) => {
          const project = card.slug ? bySlug.get(card.slug) : undefined;
          return (
            <article
              key={`${card.src}-${i}`}
      ...
      <FocusedProject project={active} onClose={() => setActive(null)} />
    </>
  );
}
