...
export default function ShipFlight() {
  ...
  return (
    ...
    >
      ...
      {/* the word — scatters apart as it dissolves */}
      <motion.span
        aria-label="ship"
        className="absolute inset-0 flex items-center justify-center"
        animate={{ opacity: [1, 1, 0, 0, 1], scale: [1, 1, 1.15, 1, 1], filter: ["blur(0px)", "blur(0px)", "blur(2px)", "blur(0px)", "blur(0px)"] }}
        transition={{ ...LOOP, times: [0, 0.46, 0.52, 0.92, 1] }}
      >
        ship
      </motion.span>

      {/* fragments: burst out from the word, then rush back in to form the plane */}
      {BLOCKS.map((b, i) => (
        <motion.span
          key={i}
          ...
          animate={{
            x: [0, 0, 0, b.x, 0, 0],
            y: [0, 0, 0, b.y, 0, 0],
            rotate: [0, 0, 0, b.x > 0 ? 90 : -90, 0, 0],
            opacity: [0, 0, 1, 1, 0, 0],
          }}
          transition={{ ...LOOP, times: [0, 0.46, 0.54, 0.64, 0.72, 1] }}
        />
      ))}
      ...
      <motion.span
        ...
        animate={{
          ...
          scale: [0.4, 0.4, 1, 1, 1, 1],
          opacity: [0, 0, 1, 1, 0, 0],
        }}
        transition={{ ...LOOP, times: [0, 0.68, 0.74, 0.86, 0.93, 1] }}
      >
